...
- **Empirical evidence** from sandbox-smoke logs: `skipping resource/memory-balloon: memory.max NOT enforced on this host. Empirical evidence: dd exited with code 0 after allocating 67108864B against a 8388608B cap`. The probe correctly identified the kernel/Docker/swap interaction *with concrete numeric evidence* — not a guess. The `resource_memory_balloon_blocked` test now passes as `ok` (self-skip is the intended branch on this runner) and the new unit test `probe_memory_max_enforcement_skipped_when_unprivileged` also passes in the unprivileged `cargo test` job.
...
- **Concerns**: the probe is not yet wired into `assert_kernel_floor`. A misconfigured production host (Docker without proper memory delegation) could still set `SandboxMode=Enforce` and silently get no memory cap. **G1c.x.2 (preflight wiring) remains the right immediate follow-up** — it converts this from a *test-time* probe to a *startup-time* fail-fast.
...
- **Evidence**: CI run `25142973309` on `91bc07f` produced 9 passed + 1 failed in `sandbox_escape_test`, plus a clippy lint failure on the new eprintln (uninlined format args). The memory-balloon test still panicked at the assertion: `memory_controller_delegated()` returned `true` (i.e. `memory` IS listed in `/sys/fs/cgroup/cgroup.subtree_control` on the runner), yet the kernel still did not OOM-kill `dd`'s 600 MiB allocation against the 512 MiB cap. The controller is *delegated* but the cap is *not enforced* — likely a Docker-overlayfs / `memory.swap.max` / privileged-container interaction.
...
- **Root cause (round 3)**: The CI runner does not delegate the `memory` controller to children of `/sys/fs/cgroup/`. cgroup v2 enforces a *controller-delegation contract*: a controller can only enforce limits on children of a cgroup if that controller is enabled in the cgroup's `cgroup.subtree_control`. Writing to `memory.max` on a child cgroup whose parent does NOT have `+memory` in subtree_control succeeds at the file level (the kernel accepts the write) but the limit is a no-op. `pids` is delegated on this runner (proven by `cgroup_pids_max_limits_fork_count` and our own fork-bomb / pid-exhaustion tests biting the cap); `memory` is not. The existing `cgroup_v2_writable()` precondition only checks `mkdir`, not per-controller delegation, which is why the gap was not surfaced until now.
