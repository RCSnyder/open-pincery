...
- **AC-76 / G1c memory-balloon end-to-end CI verification** — the resource-category memory-balloon payload (`dd bs=600M count=1` exceeds `memory.max=512 MiB`) self-skips on hosts where the cgroup v2 `memory` controller is not delegated to children of `/sys/fs/cgroup/` (i.e. `memory` is not in `cgroup.subtree_control`). The current privileged CI sandbox-smoke runner falls into that bucket: pids.max is enforced, but memory.max is not. The runtime code in `src/runtime/sandbox/cgroup.rs` writes to `memory.max` correctly; what's missing is the host-side delegation. **Followup**: either (a) add a runtime-level check that fails fast in Enforce mode when the memory controller is not delegated (preserving the AC-53 contract by refusing to run rather than silently degrading), or (b) update the CI runner image to delegate the memory controller (`echo +memory > /sys/fs/cgroup/cgroup.subtree_control` in the privileged container's entrypoint). Until then, the memory-balloon test is the only AC-76 payload that self-skips on this runner; fork-bomb and pid-exhaustion exercise the same cgroup hierarchy via pids.max successfully.
