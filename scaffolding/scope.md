...
- **AC-84** (Kernel ABI Floor Preflight — fail-closed at server start, not per-call): A new `src/runtime/sandbox/preflight.rs::assert_kernel_floor()` runs once at `pincery-server` startup and asserts: (a) Landlock ABI ≥ 6 via `landlock::ABI::new_current()`; (b) seccomp-bpf available (`prctl(PR_GET_SECCOMP) != -1`); (c) cgroup v2 mounted at `/sys/fs/cgroup`; (d) `/proc/sys/kernel/unprivileged_userns_clone == 1` OR caller is root; (e) `bwrap --version` ≥ 0.8.0 present on `$PATH`. Any failure aborts startup with a `sandbox_kernel_floor_unmet` log record naming the missing requirement and exit code 4 (distinct from existing exit codes). The ABI floor (`6`) is a constant in `preflight.rs` and is documented in [`preferences.md`](preferences.md) and `DELIVERY.md` "System Requirements" as **Linux ≥ 6.7** (Landlock ABI 6 ships in 6.7). Operators on older kernels can opt out via `OPEN_PINCERY_SANDBOX_FLOOR=relaxed`, which downgrades to ABI ≥ 1 with a startup `sandbox_floor_relaxed` warning event AND requires `OPEN_PINCERY_ALLOW_UNSAFE=true`. Verified by `tests/sandbox_preflight_test.rs`: (1) on a compliant kernel, preflight returns `Ok`; (2) a stubbed `KernelProbe::abi() = 5` returns `Err(LandlockTooOld)` and the binary exits 4; (3) `OPEN_PINCERY_SANDBOX_FLOOR=relaxed` without `ALLOW_UNSAFE=true` still exits 4. Binds canonical action `AttestSandbox`.
