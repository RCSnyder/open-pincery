...
- New `pcy login` semantics — idempotent. On first run with no existing admin it calls `POST /api/bootstrap`; on subsequent runs it calls `POST /api/login`. The separate `pcy bootstrap` subcommand is kept as a hidden alias that emits a deprecation warning to stderr and delegates to `login`.
...
- **AC-45** (`pcy login` is idempotent): `pcy login` succeeds against both a fresh server and an already-bootstrapped server. On a fresh server with `OPEN_PINCERY_BOOTSTRAP_TOKEN` set, it calls `POST /api/bootstrap` and persists the returned session token. On an already-bootstrapped server with the same env set, it calls `POST /api/login` and persists the returned session token. On either path the terminal output is a single line `Logged in to <context> as <email>`. The separate `pcy bootstrap` subcommand is removed from `--help` and kept as a hidden alias that emits `warning: 'pcy bootstrap' is deprecated, use 'pcy login' instead.` to stderr and delegates to `login`. Verified by `tests/cli_login_idempotent_test.rs`: (a) against a freshly-reset compose stack, `pcy login` succeeds and a subsequent `pcy whoami` returns 200; (b) a second `pcy login` on the same stack also succeeds (does not 409); (c) `pcy bootstrap --bootstrap-token $T` still works and emits exactly one deprecation warning line to stderr with stdout outcome identical to `pcy login`; (d) `pcy --help` stdout contains `login` but not `bootstrap`.
