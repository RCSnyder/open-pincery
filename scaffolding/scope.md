...
- **AC-76 / G1c memory-balloon end-to-end CI verification** — the resource-category memory-balloon payload (`dd bs=600M count=1` exceeds `memory.max=512 MiB`) self-skips on hosts where cgroup v2 `memory.max` is not actually enforced by the kernel. As of G1c.x (commit pending), this gating is **empirical** rather than heuristic: `cgroup::probe_memory_max_enforcement()` creates an 8 MiB-capped cgroup, runs `dd bs=64M count=1` attached to it, and observes whether the kernel SIGKILLs the over-allocator. The probe correctly distinguishes three states: `Enforced` (test runs and asserts), `NotEnforced` (test self-skips with concrete evidence — covers the privileged-Docker-CI case where `memory` is in `subtree_control` but the kernel still does not OOM-kill, observed in CI runs 25142773968 / 25142973309), and `Skipped` (cgroup v2 not writable). **Remaining followup**: either (a) extend the probe into a startup gate in `assert_kernel_floor` that refuses `Enforce` mode when the probe says `NotEnforced` (preserves AC-53 contract by refusing to run rather than silently degrading), or (b) update the CI runner image to actually enforce memory.max (privileged container entrypoint that disables swap accounting / sets `memory.swap.max=0`). G1c.x ships only the probe + test gating; the startup-gate posture decision is deferred to G1c.x.2.
