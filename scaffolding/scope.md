...
- **AC-86** (UID Drop + Capability Drop in Bwrap — close attack surface inside the userns): `build_bwrap_args` adds three flags to every sandbox launch: `--uid 65534 --gid 65534 --cap-drop ALL`. The sandboxed process therefore starts as `nobody:nogroup` with the empty capability set inside the userns, eliminating `CAP_SYS_ADMIN`-in-userns nested-namespace exploits (CVE-2022-0185, CVE-2023-32233 class), `CAP_NET_RAW` ARP/ICMP spoofing, and `CAP_SYS_PTRACE` cross-PID-ns probes. `pincery-init` (AC-83) re-asserts `setresuid(65534)` + `setresgid(65534)` + `setgroups(0, NULL)` + `capset(empty)` as defense-in-depth. The configured `OPEN_PINCERY_SANDBOX_UID` / `_GID` env vars override 65534 when an operator needs a different non-zero UID; UID 0 is rejected at preflight unless `OPEN_PINCERY_ALLOW_UNSAFE=true`. Verified by `tests/sandbox_uid_drop_test.rs`: (1) `id -u` inside the sandbox returns `65534`; (2) `cat /proc/self/status | grep CapEff` returns `0000000000000000`; (3) `unshare -U` inside the sandbox returns EPERM; (4) attempting `OPEN_PINCERY_SANDBOX_UID=0` without `ALLOW_UNSAFE=true` aborts startup. Binds canonical action `ProvisionSandbox`.
