...
- **AC-88** (Kernel Audit Integration — observe what the agent tried to do): `pincery-init` passes `LANDLOCK_RESTRICT_SELF_LOG_NEW_EXEC_ON` (ABI ≥ 7) so the kernel emits `AUDIT_LANDLOCK_*` records when the sandboxed process attempts a denied operation. A new background reader `src/observability/landlock_audit.rs` parses the kernel audit netlink (or `/var/log/audit/audit.log` fallback when systemd-journald is configured) and translates each record into a structured `landlock_denied` event in our event log with `{tool_name, agent_id, denied_path, requested_access, syscall}`. On kernels with ABI < 7, the reader is a no-op and a one-time `audit_log_unavailable` startup warning is emitted (forensics degraded but sandbox still enforced). Verified by `tests/landlock_audit_test.rs`: (1) trigger a denied open inside the sandbox; assert a `landlock_denied` event appears within 2s with the correct path; (2) on stubbed ABI 6, assert the reader does not start and the warning event is emitted. Binds canonical action `AttestSandbox`. Adds one new event type `landlock_denied`.
...
- `landlock_denied`, `audit_log_unavailable` (AC-88)
