...
function statusClass(status) {
  ...
  if (status === "maintenance" || status === "wake_ending")
...
function toast(message, isError = false) {
  ...
  const el = h(
    ...
    h(
      "span",
      { style: isError ? "color: var(--danger)" : "color: var(--accent)" },
      isError ? "✗ " : "✓ "
    ),
    message
  )
...
function renderAuth() {
  const form = h(
    "form",
    { class: "form", onSubmit: handleBootstrap },
    h(
      "label",
      { class: "label" },
      "Bootstrap Token",
    ...
    h(
      "label",
      { class: "label", style: "margin-top: 12px" },
  ...
  return h(
    ...
    h(
        ...
        h(
          ...
          h(
            ...
            h(
              "span",
              { class: "muted" },
              "Compare-and-swap state transitions prevent race conditions"
            )
          ),
          h(
            "div",
            ...
            h(
              "span",
              { class: "muted" },
              "Every action is an immutable, auditable event"
            )
          ),
          h(
            "div",
            ...
            h(
              "span",
              { class: "muted" },
              "Agents build identity and work lists through maintenance cycles"
            )
          )
        )
      ),
      h(
        ...
        h(
          "h2",
          { style: "margin: 0 0 8px; font-family: var(--font-display)" },
          "Get Started"
        ),
        h(
          "p",
          { class: "card-copy", style: "margin-bottom: 20px" },
  ...
  const sidebar = h(
    ...
    h(
      ...
      h(
        "p",
        { class: "brand-copy" },
        "Autonomous agent runtime with CAS lifecycle, event sourcing, and self-maintenance."
      )
    ),
    h(
      "div",
...
function renderOverview() {
  ...
  const awake = state.agents.filter(
    a => a.status === "awake" || a.status === "wake_acquiring"
  ).length
  const resting = state.agents.filter(
    a => a.status === "resting" || a.status === "asleep"
  ).length
  const maintenance = state.agents.filter(
    a => a.status === "maintenance" || a.status === "wake_ending"
  ).length
    ...
    h(
      ...
      h(
        ...
        h(
          ...
          h(
            "span",
            { class: "metric-value", style: "color: var(--accent-2)" },
            String(awake)
          )
        )
      ),
              ...
              h(
                ...
                h(
                  ...
                  h(
                    ...
                    { class: `pill` },
                    h("span", {
                      class: `status-dot ${state.health.status === "ok" ? "is-awake" : "is-maintenance"}`
                    }),
                    state.health.status
                  )
                )
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h(
          ...
          {
            style:
              "font-family: var(--font-display); margin: 0 0 4px; letter-spacing: -0.03em"
          },
          a.name
        ),
        h(
    ...
    h(
      ...
      h(
        ...
        h(
          "form",
          { class: "form", onSubmit: handleSendMessage },
          h("textarea", {
            class: "textarea",
            id: "message-input",
    ...
    h(
      "div",
      { class: "card span-12", style: "grid-column: span 12" },
      h(
        "div",
        {
          style:
      ...
      state.events.length > 0
            ...
            ...state.events.map(ev =>
              h(
                ...
                h(
                  ...
                  h(
                    ...
                    h(
                      "span",
                      { class: "event-type" },
                      `${eventTypeIcon(ev.event_type)} ${ev.event_type}`
                    ),
                ...
                ev.tool_name
                  ? h(
                      "div",
                      { class: "event-content", style: "margin-top: 8px" },
                      ...
                      ev.tool_input
                        ? h(
                            ...
                            {
                              style:
                                "margin: 6px 0 0; font-size: 12px; color: var(--text-muted); white-space: pre-wrap; word-break: break-word"
                            },
                            ev.tool_input
                          )
                        : null,
                      ev.tool_output
                        ? h(
                            ...
                            {
                              style:
                                "margin: 6px 0 0; font-size: 12px; color: var(--accent); white-space: pre-wrap; word-break: break-word"
                            },
                            ev.tool_output
                          )
                        : null
                ...
                ev.termination_reason
                  ? h(
                      ...
                      {
                        style:
                          "margin-top: 6px; color: var(--warning); font-size: 12px"
                      },
        ...
        : h(
            "div",
            { class: "empty-state" },
            "No events recorded yet. Send a message to get started."
          )
    )
  )
}
