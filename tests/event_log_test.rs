...
async fn test_event_log_append_and_query() {
    ...
    let u = user::create_local_admin(&pool, "evtest@test.com", "EvTest")
        .await
        .unwrap();
    let org = workspace::create_organization(&pool, "evtest", "evtest", u.id)
        .await
        .unwrap();
    let ws = workspace::create_workspace(&pool, org.id, "evtest", "evtest", u.id)
        .await
        .unwrap();
    let a = agent::create_agent(&pool, "ev-agent", ws.id, u.id)
        .await
        .unwrap();
    ...
    // has_pending_events
    let has = event::has_pending_events(&pool, a.id, e1.created_at)
        .await
        .unwrap();
