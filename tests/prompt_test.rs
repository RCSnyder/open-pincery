...
async fn test_prompt_assembly() {
    ...
    let u = user::create_local_admin(&pool, "prompt@test.com", "Prompt").await.unwrap();
    let org = workspace::create_organization(&pool, "prompt", "prompt", u.id).await.unwrap();
    let ws = workspace::create_workspace(&pool, org.id, "prompt", "prompt", u.id).await.unwrap();
    let a = agent::create_agent(&pool, "prompt-agent", ws.id, u.id).await.unwrap();

    // Add a message event
    open_pincery::models::event::append_event(
        &pool, a.id, "message_received", "human", None, None, None, None, Some("What's up?"), None,
    ).await.unwrap();

    let assembled = prompt::assemble_prompt(&pool, a.id, 200, 20, 100000).await.unwrap();
