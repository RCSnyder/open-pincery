...
async fn test_shutdown_cancels_stale_recovery() {
    ...
    let config = std::sync::Arc::new(open_pincery::config::Config::from_env().unwrap_or_else(|_| {
        open_pincery::config::Config {
    ...
    assert!(result.is_ok(), "Stale recovery should exit within 2 seconds after cancellation");
    assert!(result.unwrap().is_ok(), "Stale recovery should exit cleanly");
}
